...
class OrderControllerTest {
    ...
    class GetStoreOrdersAuthorizationTest {
        ...
        void owner_can_access_store_orders() throws Exception {
            ...
            // when & then
            mockMvc.perform(
                            get("/api/v1/stores/{storeId}/orders", storeId)
                                    .with(user(owner))
                    )
                    .andExpect(status().isOk());

            // Controller가 인증 사용자 ID와 역할 목록을 Service에 정확히 전달했는지 확인한다.
            verify(orderService).getStoreOrders(
                    eq(storeId),
                    eq(ownerId),
        ...
        void admin_can_access_store_orders(
                String role
        ) throws Exception {
    ...
    class GetOrderAuthorizationTest {
        ...
        void allowed_roles_can_access_order_detail(
                String role
        ) throws Exception {
        ...
        void unauthenticated_user_cannot_access_order_detail()
                throws Exception {
    ...
    class ChangeStoreOrderStatusAuthorizationTest {
        ...
        static Stream<Arguments> storeStatusChangeCases() {
            return Stream.of(
                    ...
                    Arguments.of(
                            "/api/v1/stores/{storeId}/orders/{orderId}/cook",
                            OrderStatus.COOKING
                    ),
                    Arguments.of(
        ...
        void owner_can_access_store_status_change_apis(
                String path,
             ...
             * Service에 전달했는지 확인한다.
             */
            verify(orderService).changeStoreOrderStatus(
                    storeId,
                    orderId,
                    nextStatus,
                    ownerId,
        ...
        @MethodSource("storeStatusChangeCases")
        void customer_cannot_access_store_status_change_apis(
                String path,
                OrderStatus nextStatus
        ) throws Exception {
            // given
        ...
        @ValueSource(strings = {"MANAGER", "MASTER"})
        void admin_can_access_store_status_change_api(
                String role
        ) throws Exception {
