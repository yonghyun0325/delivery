...
public interface RegionControllerDocs {
    ...
    ResponseEntity<RestApiResponse<RegionResponse>> updateRegion(UUID regionId, RegionRequest request);

    @Operation(summary = "지역 삭제", description = "지역을 소프트 삭제 처리합니다.")
    ResponseEntity<RestApiResponse<Void>> deleteRegion(UUID regionId, CustomUserDetails userDetails);
}