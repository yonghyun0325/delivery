...
public interface CategoryControllerDocs {
    ...
    ResponseEntity<RestApiResponse<CategoryResponse>> updateCategory(
            UUID categoryId, CategoryRequest request);

    @Operation(summary = "카테고리 삭제", description = "카테고리를 소프트 삭제 처리합니다.")
    ResponseEntity<RestApiResponse<Void>> deleteCategory(
            UUID categoryId, CustomUserDetails userDetails);
}
