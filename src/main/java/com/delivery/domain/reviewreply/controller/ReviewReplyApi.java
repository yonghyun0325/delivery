...
public interface ReviewReplyApi {
    ...
    @Operation(
            summary = "리뷰 답글 등록",
            description = "해당 음식점의 실제 사장님이 리뷰에 답글을 등록합니다.")
    @ApiResponses({
    ...
    ResponseEntity<RestApiResponse<ReviewReplyResponse>> createReply(
            ...
            @Parameter(description = "답글을 등록할 리뷰 ID", required = true)
            @PathVariable
            UUID reviewId,
    ...
    @Operation(
            summary = "리뷰 답글 조회",
            description = "리뷰 ID를 기준으로 삭제되지 않은 사장님 답글을 조회합니다.")
    @ApiResponses({
    ...
    ResponseEntity<RestApiResponse<ReviewReplyResponse>> getReply(
            @Parameter(description = "답글을 조회할 리뷰 ID", required = true)
            @PathVariable
            UUID reviewId);

    @Operation(
            summary = "리뷰 답글 수정",
            description = "답글을 작성한 사장님 본인이 답글 내용을 수정합니다.")
    @ApiResponses({
    ...
    ResponseEntity<RestApiResponse<ReviewReplyResponse>> updateReply(
            ...
            @AuthenticationPrincipal
            CustomUserDetails userDetails,
            @Parameter(description = "답글이 작성된 리뷰 ID", required = true)
            @PathVariable
            UUID reviewId,
    ...
    @Operation(
            summary = "리뷰 답글 삭제",
            description = "답글을 작성한 사장님 본인이 답글을 소프트 삭제합니다.")
    @ApiResponses({
