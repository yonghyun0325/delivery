...
public interface ReviewApi {
    ...
    @Operation(
            summary = "리뷰 등록",
            description = "배송 완료 또는 주문 완료 상태의 주문에 대해 리뷰를 등록합니다.")
    @ApiResponses({
    ...
    @Operation(
            summary = "리뷰 상세 조회",
            description = "리뷰 ID를 이용하여 삭제되지 않은 리뷰의 상세 정보를 조회합니다.")
    @ApiResponses({
    ...
    @Operation(
            summary = "리뷰 수정",
            description = "리뷰 작성자 본인이 평점과 리뷰 내용을 수정합니다.")
    @ApiResponses({
    ...
    @Operation(
            summary = "리뷰 삭제",
            description = "리뷰 작성자 본인이 리뷰를 소프트 삭제합니다.")
    @ApiResponses({
    ...
    @Operation(
            ...
            description = "음식점 ID를 기준으로 리뷰 목록을 페이징 및 정렬하여 조회합니다.")
    @ApiResponses({
    ...
    @Operation(
            summary = "내 리뷰 목록 조회",
            description = "로그인한 사용자가 작성한 삭제되지 않은 리뷰 목록을 조회합니다.")
    @ApiResponses({
    ...
    @Operation(
            summary = "음식점 평균 평점 조회",
            description = "음식점 ID를 기준으로 삭제되지 않은 리뷰의 평균 평점을 조회합니다.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "음식점 평균 평점 조회 성공")
    })
    ResponseEntity<RestApiResponse<Double>> getStoreRating(
            @Parameter(description = "평균 평점을 조회할 음식점 ID", required = true)
            @PathVariable
            UUID storeId);
}